...
function JumpToTODO(forward)
  ...
  -- Primary search
  if forward then
    target_line, target_col = search_range(current_line + 1, total_lines, 1)
  else
    target_line, target_col = search_range(current_line - 1, 1, -1)
  end

  -- Wrap-around search if nothing found
  if not target_line then
    if forward then
      target_line, target_col = search_range(1, current_line, 1)
    else
      target_line, target_col = search_range(total_lines, current_line, -1)
    end

    if target_line then
      print 'Wrapped TODO search.'
    end
  end
